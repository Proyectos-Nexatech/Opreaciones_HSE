...
interface EmpresaClienteModalProps {
    isOpen: boolean;
    onClose: () => void;
...
export const EmpresaClienteModal: React.FC<EmpresaClienteModalProps> = ({
    isOpen,
    onClose,
    onSave,
    initialData
            ...
            <div className="bg-white rounded-[40px] w-full max-w-md overflow-hidden shadow-2xl border border-white/20 animate-in zoom-in-95 duration-300">
                <div className="bg-brand-primary p-8 text-white flex justify-between items-center relative overflow-hidden">
                    ...
                    <div className="relative z-10 flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-black tracking-tight">{initialData ? 'Editar Empresa' : 'Nueva Empresa'}</h2>
                            <p className="text-white/70 text-xs font-bold uppercase tracking-[0.2em]">{initialData ? 'Actualizar aliado' : 'Añadir aliado corporativo'}</p>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                    <div className="space-y-4">
                        <div className="space-y-1.5">
                            <label className="text-[11px] font-black text-brand-text-muted uppercase tracking-widest ml-1">Nombre de la Empresa</label>
                            <input
                                required
                                type="text"
                                value={formData.name}
                                onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                                placeholder="Ej: NEXATECH S.A.S"
                                className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-5 py-4 text-sm font-bold text-brand-text focus:outline-none focus:ring-4 focus:ring-brand-primary/10 focus:border-brand-primary transition-all placeholder:text-gray-300 shadow-inner uppercase"
                            />
                        </div>
                    </div>
